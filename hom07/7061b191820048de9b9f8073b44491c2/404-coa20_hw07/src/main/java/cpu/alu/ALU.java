...
//    private String OF = "0";
    ...
    private String oneAdder (String operand){
        ...
        int [] num = new int[len];
        for(int i=0;i<len;i++)
            num[i] = temp.charAt(i)-'0';  //先转化为反转后对应的int数组
        int bit = 0x0;
        int carry = 0x1;
    ...
    String serialCarryAdder(String adder1,String adder2){
        ...
        char carry='0';
        //output的第一位用来保存进位，当然类似的做法还有很多了，问题不大
        String output="00";
    ...
    String carryLookAheadAdder_8(String adder1,String adder2, char carryIn){
        //carryIn对应的就是ppt上面的C0，部分先行算法中每一次都要做
        ...
        for (int i = 0; i <8 ; i++) {
            //加法的本质是位运算：或运算/与运算
            //数组p用来保存两个数的加和（不考虑carry）：两个数相同加和就是0，不同就是1
            //数组g用来保存两个数的进位：只有两个数都是1才产生进位
            p[i]=x[i]|y[i];
            g[i]=x[i]&y[i];
        }
        ...
        for (int i = 0; i <8 ; i++) {
            ...
            int temp=1;
            for (int j = i ;j >=1 ; j--) {
                //这个temp相当于用来保存P3P2P1的与结果，依据循环不断更新，不断加上去新的Pi
                temp=temp&p[j];
                c[i]=c[i]|(temp&g[j-1]);
            }
        ...
        //返回的结果是9位的，最高位是八位加法的进位，剩下8位是结果
        return result.toString();


        //部分先行的算法思路：
        //部分先行主要的设计在于对于每一位进位进行先行处理
        //首先是获得两个数组p和q，分别用来保存按位或和按位与的结果
        //然后计算每一位的进位，C0已知，其他的都要算，利用公式写一个循环
        //然后计算结果，依据s=xi异或yi异或ci-1来计算，先在时钟周期里计算xi异或yi
        //然后再计算和ci-1的异或结果，重复八次得到8位的加法结果，然后再加上c7（最高位进位结果）返回
    }

    String connectedCLAAdder(String adder1,String adder2){
        //部分先行，也就是CLA每次只做8个，一共要做4次
        String result="";
    ...
    private String fullAdder(char x, char y, char c) {
        //全加器的原理ppt第四页。分别产生Si和Ci，以字符串形式返回
